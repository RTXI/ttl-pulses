...
TTL::TTL(void) :
  DefaultGUIModel("TTL", ::vars, ::num_vars),
      dt(RT::System::getInstance()->getPeriod() * 1e-9), ttlDuration(.25),
      ttlNumPulses(1), ttlFreq(1), ttlDelay(.5), maxtrials(1), trial(0), idx(0)
{
  QWhatsThis::add(
      this,
...
void
TTL::update(DefaultGUIModel::update_flags_t flag)
{
  switch (flag)
    {
  case INIT:
    ...
    break;
  case MODIFY:
    if (getParameter("TTL Duration (s)").toDouble() >= (getParameter(
        "TTL Freq (Hz)").toDouble()))
      {
